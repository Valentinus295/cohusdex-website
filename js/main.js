/* CohusDex — JS (matching Faidaza patterns) */
(function(){
'use strict';
...
function animateCounter(el,target,duration){if(target===0){el.textContent='0';return}var start=0,startTime=null;function step(ts){if(!startTime)startTime=ts;var p=Math.min((ts-startTime)/duration,1),e=1-Math.pow(1-p,3);el.textContent=Math.floor(e*target).toLocaleString();if(p<1)requestAnimationFrame(step);else el.textContent=target.toLocaleString()}requestAnimationFrame(step)}
